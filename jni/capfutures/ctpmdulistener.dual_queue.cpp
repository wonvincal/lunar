...
char* g_ringBuffer2 = 0;
unsigned int g_ringBufferWriteCursor2 = 0;
unsigned int g_ringBufferReadCursor2 = 0;
std::atomic<unsigned int> g_ringBufferCount2(0);
...
void process(const struct timeval& tv, char* data, unsigned int size) {
    ...
    if (type == EI6::DataType::EnumMarket) {
        ...
        if (iObCache == m_orderBookCaches.end()) {
            ...
            // 1 + 2 * 5 + 1 + 2 * 5 = 1 + 10 + 1 + 10 = 22
            obCache = new uint64_t[22];
            memset(obCache, 0, sizeof(uint64_t)*22);
...
void received(void* ud, const void* data, unsigned int size) {
    ...
    if (type == EI6::DataType::EnumMarket || type == EI6::DataType::EnumProduct) {
        struct timeval tv;
        ...
        memcpy(cursor, &tv, sizeof(struct timeval));
...
void received2(void* ud, const void* data, unsigned int size) {
    typedef EI6::DataType::Type  msg_type; //ei6 head file: const_def
    msg_type type = *(msg_type const*)data;
    ...
    if (type == EI6::DataType::EnumMarket || type == EI6::DataType::EnumProduct) {
        struct timeval tv;
        gettimeofday(&tv, NULL);
        while (g_ringBufferCount2 == RING_BUFFER_LENGTH) {
            // busy spin - i don't expect the code to reach here often
        }
        char* cursor = g_ringBuffer2 + g_ringBufferWriteCursor2;
        memcpy(cursor, &tv, sizeof(struct timeval));
        cursor+=sizeof(struct timeval);
        memcpy(cursor, &size, sizeof(unsigned int));
        cursor+=sizeof(unsigned int);
        memcpy(cursor, data, size);
        g_ringBufferWriteCursor2 = (g_ringBufferWriteCursor2 + PACKET_SIZE) & RING_BUFFER_MASK;
        g_ringBufferCount2++;
    }
}
...
void* consume(void* arg) {
    ...
    while (canConsume) {
        ...
        if (g_ringBufferCount2 > 0) {
            char* cursor = g_ringBuffer2 + g_ringBufferReadCursor2;
            memcpy(&tv, cursor, sizeof(struct timeval));
            cursor+=sizeof(struct timeval);
            memcpy(&size, cursor, sizeof(unsigned int));
            cursor+=sizeof(unsigned int);
            process(tv, cursor, size);
            g_ringBufferReadCursor2 = (g_ringBufferReadCursor2 + PACKET_SIZE) & RING_BUFFER_MASK;
            g_ringBufferCount2--;
        }
...
void initialize() {
    ...
    g_ringBuffer2 = new char[RING_BUFFER_SIZE];
    g_ringBufferWriteCursor2 = 0;
    g_ringBufferReadCursor2 = 0;
    g_ringBufferCount2.store(0);
...
int main(int argc, char* argv[]) {
	...
	*(void**)&unload = dlsym(dll_handle, "ei6_unload");
	...
	void* gateway_omdc = load(config_file_omdc, received, &OMDC);
    void* gateway_omdd = load(config_file_omdd, received2, &OMDD);
    pause();
	unload(gateway_omdc);
    unload(gateway_omdd);
	dlclose(dll_handle);
