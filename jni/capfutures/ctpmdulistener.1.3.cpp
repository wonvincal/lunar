...
void process(const struct timeval& tv, const void* data, unsigned int size) {
    ...
    if(type == EI6::DataType::EnumMarket){
		...
		if (msg.has_DateTime() && msg.has_ProductID()) {
			...
			if (msg.has_ExchangePlatformLastTradeStatus()) {
				if (msg.get_ExchangePlatformLastTradeStatus()[0] == 'N') {
					if (msg.has_LastTradePrice()) {
						if (msg.has_ExchangePlatformLastTradeType() && (msg.get_ExchangePlatformLastTradeType()[0] == '0' || strcmp(msg.get_ExchangePlatformLastTradeType(), "100") == 0)) {
							...
							std::cout << msg.get_ProductID() << "," << "Trade,," << tradePrice << "," << msg.get_LastTradeQuantity() << "," << 0 << ",";
							...
							for (int idx = 0; idx < 5; idx++) {
								if (idx < depthSize) {
									...
									int price = decimalToInt(bidSideEntry->GetPrice());
									if (idx == 0)
										bestBid = price;
									if (bidSideEntry->HasPrice() && bidSideEntry->HasQuantity() && bidSideEntry->GetPrice() > 0) {
										std::cout << price << "," << bidSideEntry->GetQuantity() << ",";
										continue;
									}
								}
								std::cout << ",,";
							...
							for (int idx = 0; idx < 5; idx++) {
								if (idx < depthSize) {
									...
									int price = decimalToInt(askSideEntry->GetPrice());
									if (askSideEntry->HasPrice() && askSideEntry->HasQuantity() && askSideEntry->GetPrice() > 0) {
										std::cout << price << "," << askSideEntry->GetQuantity() << ",";
										continue;
									}
								}
								std::cout << ",,";
				...
				struct tm* n = gmtime(&t);
				std::cout << n->tm_year + 1900 << "-" << std::setfill('0') << std::setw(2) << n->tm_mon + 1 << "-" << std::setfill('0') << std::setw(2) << n->tm_mday << " " << std::setfill('0') << std::setw(2) << n->tm_hour + 8 << ":" << std::setfill('0') << std::setw(2) << n->tm_min << ":" << std::setfill('0') << std::setw(2) << n->tm_sec << "." << std::setfill('0') << std::setw(6) << tv.tv_usec << ",";
				std::cout << msg.get_ProductID() << "," << "MarketDepth,,,,,";
				std::vector<EI6::MarketDepth> depth = msg.get_Depth();
				int depthSize = msg.get_Depth().size();
				for (int idx = 0; idx < 5; idx++) {
					if (idx < depthSize) {
        ...
        uint32_t rawSize = msg.get_RawMessage().size;
