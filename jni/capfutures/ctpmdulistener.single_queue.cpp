...
void received(void* ud, const void* data, unsigned int size) {
    ...
    if (type == EI6::DataType::EnumMarket || type == EI6::DataType::EnumProduct) {
        ...
        if (numNewEntries > 0) {
            ...
            //char str[255];
            //sprintf(str, "[DEBUG] MarketId: %d, writeCount: %d, writtenCount: %d. Ring buffer count increased by %d to %d\n", market_id, writeCount, writtenCount, numNewEntries, count);
            //std::cout << str;
        }
        //else {
            //char str[255];
            //sprintf(str, "[DEBUG] MarketId: %d, writeCount: %d, writtenCount: %d. Ring buffer count NOT increased by %d\n", market_id, writeCount, writtenCount, numNewEntries);
            //std::cout << str;
        //}
    }
}
...
int main(int argc, char* argv[]) {
	...
	*(void**)&unload = dlsym(dll_handle, "ei6_unload");
    ...
    void* gateway_omdd = load(config_file_omdd, received, &OMDD);
    pause();
	unload(gateway_omdc);
    unload(gateway_omdd);
	dlclose(dll_handle);
