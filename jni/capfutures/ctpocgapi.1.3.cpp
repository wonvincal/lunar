...
char* g_securitiesBySid[MAX_SECURITY_SID + 1];
...
void buildSecuritiesMap() {
    ...
    for (int i = 0; i <= MAX_SECURITY_SID; i++) {
        g_securitiesBySid[i] = new char[MAX_SID_DIGITS + 1];
        sprintf(g_securitiesBySid[i], "%d", i);
    }
}

JNIEXPORT jint JNICALL Java_com_lunar_order_hkex_ocg_CtpOcgApi_initialize(JNIEnv* env, jobject obj, jstring connectorFile, jstring configFile, jstring user, jstring account, jint date) {
...
JNIEXPORT jint JNICALL Java_com_lunar_order_hkex_ocg_CtpOcgApi_addOrder(JNIEnv* env, jobject obj, jlong nanoOfDay, jstring clientOrderId, jlong secSid, jint side, jint orderType, jint timeInForce, jint price, jint quantity, jboolean isEnhanced) {
    ...
    if (!g_warmup) {
        std::cout << "[INFO] Sending order " << cClientOrderId << " " << decPrice  << "@" << quantity << "..." << std::endl;
        struct timeval starttv;
        gettimeofday(&starttv,NULL);
        g_send(g_connector, request.data(), request.size());
        struct timeval senttv;
        gettimeofday(&senttv,NULL);
        time_t t = starttv.tv_sec;
        struct tm* n = gmtime(&t);
        std::cout << "[INFO] Sending order " << cClientOrderId << " at "; 
        std::cout << std::setfill('0') << std::setw(2) << n->tm_hour + 8 << ":" << std::setfill('0') << std::setw(2) << n->tm_min << ":" << std::setfill('0') << std::setw(2) << n->tm_sec << "." <<  std::setfill('0') << std::setw(6) << starttv.tv_usec;
        std::cout << std::endl;

        t = senttv.tv_sec;
        n = gmtime(&t);
        std::cout << "[INFO] Order " << cClientOrderId << " sent at ";
        std::cout << std::setfill('0') << std::setw(2) << n->tm_hour + 8 << ":" << std::setfill('0') << std::setw(2) << n->tm_min << ":" << std::setfill('0') << std::setw(2) << n->tm_sec << "." <<  std::setfill('0') << std::setw(6) << senttv.tv_usec;
        std::cout << std::endl;
    }
    ...
    if(type == EI6::DataType::EnumExecutionReport){
        ...
        if(!msg.has_InSyncFlag()) {
            if (msg.has_ClientOrderID()) {
                time_t t = receivetv.tv_sec;
                struct tm* n = gmtime(&t);
                std::cout << "[INFO] Received execution report for " << msg.get_ClientOrderID() << " at ";
                std::cout << std::setfill('0') << std::setw(2) << n->tm_hour + 8 << ":" << std::setfill('0') << std::setw(2) << n->tm_min << ":" << std::setfill('0') << std::setw(2) << n->tm_sec << "." <<  std::setfill('0') << std::setw(6) << receivetv.tv_usec;
                std::cout << std::endl;
                ...
                if (clientId != -1) {
                    ...
                    if (msg.has_ErrorText()) {
                        std::cout << "[ERROR] Execution report " << clientId << " has error " << msg.get_ErrorText() << std::endl;
                    ...
                    struct timeval handledtv;
                    gettimeofday(&handledtv, NULL);
                   
                    time_t t = handledtv.tv_sec;
                    struct tm* n = gmtime(&t);
                    std::cout << "[INFO] Handled execution report for " << msg.get_ClientOrderID() << " at ";
                    std::cout << std::setfill('0') << std::setw(2) << n->tm_hour + 8 << ":" << std::setfill('0') << std::setw(2) << n->tm_min << ":" << std::setfill('0') << std::setw(2) << n->tm_sec << "." <<  std::setfill('0') << std::setw(6) << handledtv.tv_usec;
                    std::cout << std::endl;
                }
                else {
