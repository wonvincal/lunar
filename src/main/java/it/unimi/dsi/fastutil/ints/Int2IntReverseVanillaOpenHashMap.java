...
/**
 * An open hash map that uses the key as hash code, without applying any golden ratio in attempt to distribute the element evenly.
 * Use this if you know that your elements have a high removal rate and the removal rate increases with time.  
 * 
 * For example, orders are queued in this data structured:
 * [1001 1002 1003 1004 1005]
 * 
 * Then 1002, 1003, 1004 and 1005 are removed
 * 
 * [1001                    ]
 * 
 * 1006 arrives, with reverse hash, 1006 will be placed at the end of the queue
 * 
 * [1001                1006]
 * 
 * 1007 arrives, no collision during insert.
 * [1001 1007           1006]
 * 
 * This seems to be a pretty efficient data structure to store orders and trades
 * 
 * @author wongca
 *
 */
public class Int2IntReverseVanillaOpenHashMap extends AbstractInt2IntMap implements java.io.Serializable, Cloneable, Hash {
	...
	public Int2IntReverseVanillaOpenHashMap( final int expected, final float f ) {
	...
	public Int2IntReverseVanillaOpenHashMap( final int expected ) {
	...
	public Int2IntReverseVanillaOpenHashMap() {
	 ...
	 * @param f the load factor. */
	public Int2IntReverseVanillaOpenHashMap( final Map<? extends Integer, ? extends Integer> m, final float f ) {
		this( m.size(), f );
		putAll( m );
	}
	 ...
	 *
	 * @param m a {@link Map} to be copied into the new hash map. */
	public Int2IntReverseVanillaOpenHashMap( final Map<? extends Integer, ? extends Integer> m ) {
	 ...
	 * @param f the load factor. */
	public Int2IntReverseVanillaOpenHashMap( final Int2IntMap m, final float f ) {
		this( m.size(), f );
		putAll( m );
	}
	 ...
	 *
	 * @param m a type-specific map to be copied into the new hash map. */
	public Int2IntReverseVanillaOpenHashMap( final Int2IntMap m ) {
		this( m, DEFAULT_LOAD_FACTOR );
	}
	...
	public Int2IntReverseVanillaOpenHashMap( final int[] k, final int[] v, final float f ) {
		this( k.length, f );
	 ...
	 * @param v the array of corresponding values in the new hash map.
	 * @throws IllegalArgumentException if <code>k</code> and <code>v</code> have different lengths. */
	public Int2IntReverseVanillaOpenHashMap( final int[] k, final int[] v ) {
	...
	public int remove( final int k ) {
		...
		int pos;
		// The starting point.
		if ( ( ( curr = key[ pos = ( k & mask ) ] ) == ( 0 ) ) ) return defRetValue;
	...
	public Integer remove( final Object ok ) {
		...
		if ( ( ( k ) == ( 0 ) ) ) {
			if ( containsNullKey ) return ( Integer.valueOf( removeNullEntry() ) );
			return ( null );
		}
		int curr;
		final int[] key = this.key;
		int pos;
		// The starting point.
		if ( ( ( curr = key[ pos = ( k & mask )] ) == ( 0 ) ) ) return ( null );
		if ( ( ( curr ) == ( k ) ) ) return ( Integer.valueOf( removeEntry( pos ) ) );
		while ( true ) {
	...
	public Integer get( final Integer ok ) {
		...
		if ( ( ( k ) == ( 0 ) ) ) return containsNullKey ? ( Integer.valueOf( value[ n ] ) ) : ( null );
		int curr;
		final int[] key = this.key;
		int pos;
		// The starting point.
		if ( ( ( curr = key[ pos = ( k & mask )] ) == ( 0 ) ) ) return ( null );
		if ( ( ( k ) == ( curr ) ) ) return ( Integer.valueOf( value[ pos ] ) );
	...
	public boolean containsKey( final int k ) {
		...
		int pos;
		// The starting point.
		if ( ( ( curr = key[ pos = ( k & mask )] ) == ( 0 ) ) ) return false;
	...
	private class MapIterator {
		...
		boolean mustReturnNullKey = Int2IntReverseVanillaOpenHashMap.this.containsNullKey;
		...
		/**
		 * Need special attention to this
		 * @return
		 */
		public int nextEntry() {
			...
			final int key[] = Int2IntReverseVanillaOpenHashMap.this.key;
			for ( ;; ) {
				if ( --pos < 0 ) {
		...
		public void remove() {
			...
			if ( last == n ) {
				...
				Int2IntReverseVanillaOpenHashMap.this.remove( wrapped.getInt( -pos - 1 ) );
	...
	private final class MapEntrySet extends AbstractObjectSet<Int2IntMap.Entry> implements FastEntrySet {
		...
		public boolean contains( final Object o ) {
			...
			if ( ( ( k ) == ( 0 ) ) ) return ( Int2IntReverseVanillaOpenHashMap.this.containsNullKey && ( ( value[ n ] ) == ( ( ( e.getValue() ).intValue() ) ) ) );
			int curr;
			final int[] key = Int2IntReverseVanillaOpenHashMap.this.key;
			int pos;
			// The starting point.
			if ( ( ( curr = key[ pos = ( k & mask )] ) == ( 0 ) ) ) return false;
			if ( ( ( k ) == ( curr ) ) ) return ( ( value[ pos ] ) == ( ( ( e.getValue() ).intValue() ) ) );
			// There's always an unused entry.
			while ( true ) {
				if ( ( ( curr = key[ pos = ( pos - 1 ) & mask ] ) == ( 0 ) ) ) return false;
				if ( ( ( k ) == ( curr ) ) ) return ( ( value[ pos ] ) == ( ( ( e.getValue() ).intValue() ) ) );
		...
		public void clear() {
			Int2IntReverseVanillaOpenHashMap.this.clear();
		}
	}
	...
	private final class KeySet extends AbstractIntSet {
		...
		public boolean remove( int k ) {
			...
			Int2IntReverseVanillaOpenHashMap.this.remove( k );
			return size != oldSize;
		}
	...
	protected void rehash( final int newN ) {
		...
		for ( int j = realSize(); j-- != 0; ) {
			while ( ( ( key[ --i ] ) == ( 0 ) ) );
			if ( !( ( newKey[ pos = ( key[ i ] & mask) ] ) == ( 0 ) ) ) while ( !( ( newKey[ pos = ( pos - 1 ) & mask ] ) == ( 0 ) ) );
			newKey[ pos ] = key[ i ];
