 ...
 * @see Hash
 * @see HashCommon */
public class Long2ObjectVanillaOpenHashMap<V> extends AbstractLong2ObjectMap<V> implements java.io.Serializable, Cloneable, Hash {
	...
	public Long2ObjectVanillaOpenHashMap( final int expected, final float f ) {
	...
	public Long2ObjectVanillaOpenHashMap( final int expected ) {
	...
	public Long2ObjectVanillaOpenHashMap() {
	 ...
	 * @param f the load factor. */
	public Long2ObjectVanillaOpenHashMap( final Map<? extends Long, ? extends V> m, final float f ) {
		this( m.size(), f );
		putAll( m );
	}
	 ...
	 *
	 * @param m a {@link Map} to be copied into the new hash map. */
	public Long2ObjectVanillaOpenHashMap( final Map<? extends Long, ? extends V> m ) {
	 ...
	 * @param f the load factor. */
	public Long2ObjectVanillaOpenHashMap( final Long2ObjectMap<V> m, final float f ) {
		this( m.size(), f );
		putAll( m );
	}
	 ...
	 *
	 * @param m a type-specific map to be copied into the new hash map. */
	public Long2ObjectVanillaOpenHashMap( final Long2ObjectMap<V> m ) {
		this( m, DEFAULT_LOAD_FACTOR );
	}
	...
	public Long2ObjectVanillaOpenHashMap( final long[] k, final V[] v, final float f ) {
		this( k.length, f );
	 ...
	 * @param v the array of corresponding values in the new hash map.
	 * @throws IllegalArgumentException if <code>k</code> and <code>v</code> have different lengths. */
	public Long2ObjectVanillaOpenHashMap( final long[] k, final V[] v ) {
	...
	public V remove( final long k ) {
		...
		int pos;
		// The starting point.
		if ( ( ( curr = key[ pos = (int) k & mask ] ) == ( 0 ) ) ) return defRetValue;
	...
	public V remove( final Object ok ) {
		...
		int pos;
		// The starting point.
		if ( ( ( curr = key[ pos = (int)k & mask ] ) == ( 0 ) ) ) return ( this.defRetValue );
	...
	public boolean containsKey( final long k ) {
		...
		int pos;
		// The starting point.
		if ( ( ( curr = key[ pos = (int) k & mask ] ) == ( 0 ) ) ) return false;
	...
	private class MapIterator {
		...
		boolean mustReturnNullKey = Long2ObjectVanillaOpenHashMap.this.containsNullKey;
		...
		public int nextEntry() {
			...
			final long key[] = Long2ObjectVanillaOpenHashMap.this.key;
			for ( ;; ) {
				if ( --pos < 0 ) {
		...
		public void remove() {
			...
			if ( last == n ) {
				...
				Long2ObjectVanillaOpenHashMap.this.remove( wrapped.getLong( -pos - 1 ) );
	...
	private final class MapEntrySet extends AbstractObjectSet<Long2ObjectMap.Entry<V>> implements FastEntrySet<V> {
		...
		public boolean contains( final Object o ) {
			...
			if ( ( ( k ) == ( 0 ) ) ) return ( Long2ObjectVanillaOpenHashMap.this.containsNullKey && ( ( value[ n ] ) == null ? ( ( e.getValue() ) ) == null : ( value[ n ] ).equals( ( e.getValue() ) ) ) );
			long curr;
			final long[] key = Long2ObjectVanillaOpenHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void clear() {
			Long2ObjectVanillaOpenHashMap.this.clear();
		}
	}
	...
	private final class KeySet extends AbstractLongSet {
		...
		public boolean remove( long k ) {
			...
			Long2ObjectVanillaOpenHashMap.this.remove( k );
			return size != oldSize;
		}
	...
	protected void rehash( final int newN ) {
		...
		for ( int j = realSize(); j-- != 0; ) {
			while ( ( ( key[ --i ] ) == ( 0 ) ) );
			if ( !( ( newKey[ pos = (int) key[ i ] & mask ] ) == ( 0 ) ) ) while ( !( ( newKey[ pos = ( pos + 1 ) & mask ] ) == ( 0 ) ) );
			newKey[ pos ] = key[ i ];
	...
	@SuppressWarnings("unchecked")
	public Long2ObjectVanillaOpenHashMap<V> clone() {
		Long2ObjectVanillaOpenHashMap<V> c;
		try {
			c = (Long2ObjectVanillaOpenHashMap<V>)super.clone();
		}
