 ...
 * @param <E>
 */
public class LongEntityManager<T extends Entity> implements Loadable<T> {
	static final Logger LOG = LogManager.getLogger(LongEntityManager.class);

	private static final int DEFAULT_INITIAL_CAPACITY = 128;
	private static final float DEFAULT_LOAD_FACTOR = 0.6f;
	private final EntityLoader<T> loader;
	private final Long2ObjectLinkedOpenHashMap<T> entities;
	private EntityAddRemoveHandlerList<T> handlers;

	public LongEntityManager(int initialCapacity) {
		this(initialCapacity, new EntityAddRemoveHandler<T>() {
			@Override
		...
		}, new NullEntityLoader<T>(null, null));
	}
	
	public LongEntityManager(EntityAddRemoveHandler<T> nullHandler, EntityLoader<T> loader){
		this(DEFAULT_INITIAL_CAPACITY, nullHandler, loader);
	}
	
	public LongEntityManager(int initialCapacity, EntityLoader<T> loader) {
	    this(initialCapacity, new EntityAddRemoveHandler<T>() {
            @Override
            public void onAdd(T entity) {
            }
	...
	public LongEntityManager(int initialCapacity, EntityAddRemoveHandler<T> nullHandler, EntityLoader<T> loader){
		this.entities = new Long2ObjectLinkedOpenHashMap<T>(initialCapacity, DEFAULT_LOAD_FACTOR);
		this.handlers = new EntityAddRemoveHandlerList<T>(2, nullHandler);
		this.loader = loader;
	}
	
	public LongEntityManager<T> addHandler(EntityAddRemoveHandler<T> handler){
		this.handlers.add(handler);
		return this;
	}
	
	public LongEntityManager<T> removeHandler(EntityAddRemoveHandler<T> handler){
