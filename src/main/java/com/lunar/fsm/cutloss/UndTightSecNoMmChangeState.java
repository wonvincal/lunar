 ...
 *
 */
class UndTightSecNoMmChangeState implements State {
	private static final Logger LOG = LogManager.getLogger(UndTightSecNoMmChangeState.class);
	
	@Override
	public Transition enter(TradeContext context, StateTransitionEvent enterOnEvent) {
		LOG.trace("[cutloss] Enter UndTightSecNoMmChangeState state [secSid:{}]", box(context.market().security().sid()));
	...
	public Transition onEvent(TradeContext context, StateTransitionEvent event) {
		if (event == StateTransitionEvent.NULL){
			...
			return Transitions.in_UND_TIGHT_SEC_NO_MM_CHANGE_receive_SELL_DETECTED;
		}
		else if (event == StateTransitionEvent.DERIV_MM_CHANGE){
			return Transitions.in_UND_TIGHT_SEC_NO_MM_CHANGE_receive_DERIV_MM_CHANGE;
		}
		else if (event == StateTransitionEvent.UND_WIDE){
			return Transitions.in_UND_TIGHT_SEC_NO_MM_CHANGE_receive_UND_WIDE;
		}
	...
	public Transition onOwnSecTrade(TradeContext context, TradeSbeDecoder trade) {
		if (trade.side() == Side.BUY){
			...
			LOG.info("Received sell trade [secSid:{}, osQty:{}, price:{}, quantity:{}]", box(context.market().security().sid()), context, box(context.osQty()), trade.executionPrice(), trade.executionQty());
	...
	public Transition onUndBestLevelOrderBookUpdate(TradeContext context, long timestamp, int bestBidPrice,
		...
		if (context.market().isUndWide()){
			return onEvent(context, StateTransitionEvent.UND_WIDE);
		}
		return Transitions.NO_TRANSITION;	
	}
	...
	public String toString() {
		return "UndTightSecNoMmChangeState";
	}
}
