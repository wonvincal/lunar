 ...
 *
 */
class UndTightSecMmChangeInProgressState implements State {
	private static final Logger LOG = LogManager.getLogger(UndTightSecMmChangeInProgressState.class);
	
	@Override
	public Transition enter(TradeContext context, StateTransitionEvent enterOnEvent) {
		LOG.trace("[cutloss] Enter UndTightSecMmChangeInProgressState state [secSid:{]]", box(context.market().security().sid()));
	...
	public Transition onEvent(TradeContext context, StateTransitionEvent event) {
		if (event == StateTransitionEvent.NULL){
			...
			return Transitions.in_UND_TIGHT_SEC_MM_CHANGE_IN_PROGRESS_receive_SELL_DETECTED;
		}
		else if (event == StateTransitionEvent.DERIV_MM_CHANGE_COMPLETE){
			return Transitions.in_UND_TIGHT_SEC_MM_CHANGE_IN_PROGRESS_receive_DERIV_MM_CHANGE_COMPLETE;
		}
	...
	public Transition onOwnSecTrade(TradeContext context, TradeSbeDecoder trade) {
		if (trade.side() == Side.BUY){
			...
			LOG.debug("[cutloss] Received sell trade [secSid:{}, osQty:{}, price:{}, quantity:{}]", 
					box(context.market().security().sid()),
					context, context.osQty(), trade.executionPrice(), trade.executionQty());
	...
	public String toString() {
		return "UndTightSecMmChangeInProgressState";
	}
}
