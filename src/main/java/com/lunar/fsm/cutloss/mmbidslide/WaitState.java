...
class WaitState implements State {
	...
	public Transition enter(TradeContext context, StateTransitionEvent enterOnEvent) {
		LOG.trace("[mmbidslide] Enter WaitState state [secSid:{}]", box(context.market().security().sid()));
		if (context.osQty() == 0){
			// Calculate saved or wasted pnl
			context.calculatePnl();
			LOG.info("[mmbidslide] [secSid:{}, actualPnl:{}, theoPnl:{}]", box(context.market().security().sid()), context.actualPnl() / 1000.00, context.theoPnl() / 1000.00);
			context.reset();
