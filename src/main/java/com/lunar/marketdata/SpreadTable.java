 ...
 *
 */
public interface SpreadTable {
	
	public static final int SPREAD_TABLE_MIN_LEVEL= 1;

	public static SpreadTable of(byte id, String name, int decimalPlace, int startingPriceForHashBased, SpreadTableDetails... details){
		return new ArrayPrimaryHashSecondarySpreadTable(id, name, decimalPlace, startingPriceForHashBased, details);
	}
	
	public static SpreadTable of(byte id, String name, int decimalPlace){
	    if (decimalPlace == 0)
	        return new NoConversionSpreadTable(id, name);
		return new NoConversionWithScaleSpreadTable(id, name, decimalPlace);
	}
...
//	
//	public SpreadTable(int id, String name, int decimalPlace, int startingPriceForHashBased, SpreadTableDetails... details){
//		if (details.length <= 0){
...
//	}
	int priceToTick(int price);
	int priceToTickSize(int price);
	int tickToPrice(int tickLevel);
	SpreadTableDetails detailsAtPrice(int price);
	int decimalPlace();
	int scale();
	int maxLevel();
	byte id();
