...
public class ReplayLineHandlerEngine extends MatchingLineHandlerEngine {
    ...
    public static MatchingLineHandlerEngine of(LineHandlerConfig config, TimerService timerService, SystemClock systemClock){
        return new ReplayLineHandlerEngine(config.name(), timerService, systemClock);
    }

    ReplayLineHandlerEngine(String name, TimerService timerService, SystemClock systemClock){
        ...
        this.initializeMatchingEngine(MatchingEngineFactory.createReplayMatchingEngine(new MatchingEngine.MatchedHandler() {
            @Override
