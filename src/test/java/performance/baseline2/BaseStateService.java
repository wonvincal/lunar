...
import performance.baseline2.SbePingPongSequencedLatencyTest.ServiceLifecycleAwareInstanceBuilder;
...
public class BaseStateService implements EventHandler<Frame>, LifecycleAware {
	...
	public BaseStateService(ServiceLifecycleAwareInstanceBuilder builder){
		PerfHelper helper = new PerfHelper();
		this.messageCodec = MessageCodec.of();
		this.messageBuffer = MessageBuffer.ofSize(helper.bufferSize());
	...
	final void serviceIdleStart(){
		registerHandlers();
		if (mode == Mode.START){
			service.idleStart(serviceTracker, requestTracker, commandTracker);
		}
		else {
